...
import { handleDirectoryDecision, updateBbbLink } from "./actions";
...
export default async function ContractorVerificationPage({
  ...
  const tab = sp.tab === "credentials" ? "credentials" : "directory";
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select(
      "contractor_id, business_name, city, state, " +
  ...
  return (
    <div>
      ...
      <div className="mob-col mob-gap-sm" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <div>
          ...
          <p style={{ fontSize: "13px", color: "var(--camo-gunmetal)", marginTop: "4px" }}>
            {dirPending.length} directory pending · {credPending.length} credentials pending
          </p>
        </div>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "24px" }}>
        <Link href="/dashboard/admin/contractor-verification?tab=directory" style={tabStyle(tab === "directory")}>
          📋 Directory Verification
          {dirPending.length > 0 && (
            <span style={{ marginLeft: "8px", background: tab === "directory" ? "#fff" : "var(--camo-accent)", color: tab === "directory" ? "var(--camo-accent)" : "var(--camo-ink)", borderRadius: "20px", padding: "1px 7px", fontSize: "11px" }}>
              {dirPending.length}
            </span>
          )}
        </Link>
        <Link href="/dashboard/admin/contractor-verification?tab=credentials" style={tabStyle(tab === "credentials")}>
          📜 Licenses & Bonding
